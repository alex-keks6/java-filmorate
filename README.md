...
4. внешний ключ `status-id` (отсылает к таблице `status`) - идентификатор статуса заявки в друзья.
---
### `status`
Содержит данные о статусах заявок в друзья пользователей.

Таблица включает следующие поля:
1. первичный ключ `status_id` - идентификатор записи в данной таблице;
2. `name` - название статуса заявки. Принимает следующие значения:
